...
var zswap = tryRequire( resolve( __dirname, './../lib/ndarray.native.js' ) );
var opts = {
	'skip': ( zswap instanceof Error )
};
...
tape( 'main export is a function', opts, function test( t ) {
	...
	t.strictEqual( typeof zswap, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 7', opts, function test( t ) {
	t.strictEqual( zswap.length, 7, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports an `x` stride', opts, function test( t ) {
	...
	xe = new Float64Array( [ 0.0, 0.0, 3.0, 4.0, 0.0, 0.0, 7.0, 8.0 ] );
...
tape( 'the function supports an `x` offset', opts, function test( t ) {
	...
	xe = new Float64Array( [ 1.0, 2.0, 3.0, 4.0, 0.0, 0.0, 0.0, 0.0 ] );
...
tape( 'the function supports a `y` stride', opts, function test( t ) {
	...
	xe = new Float64Array( [ 0.0, 0.0, 0.0, 0.0, 5.0, 6.0, 7.0, 8.0 ] );
...
tape( 'the function returns a reference to the destination array', opts, function test( t ) {
	...
	out = zswap( x.length, x, 1, 0, y, 1, 0 );
...
tape( 'the function supports negative strides', opts, function test( t ) {
	...
	zswap( 2, x, -2, x.length-2, y, -1, y.length-2 );

	xe = new Float64Array( [ 0.0, 0.0, 3.0, 4.0, 0.0, 0.0, 7.0, 8.0 ] );
	ye = new Float64Array( [ 0.0, 0.0, 1.0, 2.0, 5.0, 6.0, 0.0, 0.0 ] );
...
tape( 'the function supports complex access patterns', opts, function test( t ) {
	...
	zswap( 2, x, 2, 1, y, -1, y.length-1 );

	xe = new Float64Array( [ 1.0, 2.0, 0.0, 0.0, 5.0, 6.0, 0.0, 0.0 ] );
	ye = new Float64Array( [ 0.0, 0.0, 0.0, 0.0, 7.0, 8.0, 3.0, 4.0 ] );
